...
def setup_online_evaluation(
    ...
    try:
        ...
        if evaluation:
            ...
            logger.info("Using existing Online Evaluation: %s (ID: %s)", evaluation_name, evaluation_id)
        else:
...
def setup_architecture_review_policies(
    ...
    if not gateway_arn:
        ...
        if not gateway_arn:
            logger.warning("Could not set up Gateway. Policies cannot be created without a Gateway.")
            return None
